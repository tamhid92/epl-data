...
def ensure_form(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _compute_group(g: pd.DataFrame) -> pd.DataFrame:
        ...
        for t in g["kickoff_time"]:
            ...
            val = float(window.sum()) / matches if matches > 0 else 0.0
            out.append(val)
        g = g.copy()
        g["form"] = out
        return g
...
def lightgbm_monotone_constraints(features_wo_pos: List[str]) -> List[int]:
    cons = []
    names = ["pos_num"] + features_wo_pos
    for f in names:
        cons.append(1 if f == "form" else 0)
    return cons
...
def main():
    ...
    monotone = lightgbm_monotone_constraints(feature_cols_wo_pos)
    ...
    dtrain = lgb.Dataset(X_train, label=y_train, feature_name=feature_names_with_pos)
    dvalid = lgb.Dataset(X_valid, label=y_valid, feature_name=feature_names_with_pos, reference=dtrain)
    ...
    print(f"[LGB Validation] GW={latest_gw} MAE: {mae:.3f} on {len(y_valid)} rows")
    ...
    contribs = model.predict(X_pred, num_iteration=model.best_iteration, pred_contrib=True)
